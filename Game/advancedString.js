function advancedString(string, rectangleToFitIn, colorList) {
	...
	for (var i = 0; i < string.length; i++) { 
		...
		var returnList = this.findTooltip(string, i);
		this.wordTooltipList.push(returnList.tooltipAndNumber); //console.warn(this.wordTooltipList);
		string = returnList.string;
	...
	//var boxSize = this.wordPositionList[3] - this.wordPositionList[2];
	//this.wordRectangle.push(new Rectangle(this.wordPositionList[3], 84, boxSize, 16)); 
...
advancedString.prototype.findTooltip = function(string, i) {
		...
		for (var t = 0; t < string.length; t++) { clr += string[i + t]; if ( t != 0 && string[i + t] == "&") { var point = i + t; break;} }
		clr = clr.substring(1, clr.length - 1)
		var tooltipAndNumber = { word: this.wordList.length, color: clr };
		
		var half1 = string.substring(0, i);
		var half2 = string.substring(point + 1, string.length);

		var tstring = half1 + half2;}
		var returnList = {string: tstring, tooltipAndNumber: tooltipAndNumber};
		return returnList;*/
}
...
if (string[i] == "&") {
		...
		var returnList = this.findTooltip(string, i);
		string = returnList.string;
		this.wordTooltipList.push(returnList.tooltipAndNumber);
		}*/
